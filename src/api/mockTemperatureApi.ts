...
const getStatus = (value: number): SensorStatus => {
  ...
  if (
    ...
    value >= alertSettings.highThreshold - 6
  )
    return "warning";
  return "normal";
};
...
export const mockTemperatureApi = {
  ...
  async getHistory(query: HistoryQuery): Promise<TemperaturePoint[]> {
    ...
    const targetSensors = query.sensorId
      ? sensors.filter((x) => x.sensorId === query.sensorId)
      : sensors;
    ...
    while (cursor.isBefore(end) || cursor.isSame(end)) {
      targetSensors.forEach((sensor, index) => {
        ...
        points.push({
          ...
          value: Number(
            (baseline + hourlyBias + randomBetween(-2.2, 2.2)).toFixed(1),
          ),
        });
      });
